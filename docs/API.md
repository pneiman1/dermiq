...
### `POST /chat`
Answers a natural-language question about the practice, grounded in a retrieved
knowledge corpus (built from the marts) and generated by Claude. Retrieval-
augmented — the model may only use the retrieved documents. Returns the answer plus
the documents it drew on. See ADR-008.
```bash
curl -s $BASE/chat -H "$H" -H "Content-Type: application/json" \
  -d '{"question": "Which patient segments are highest value?"}'
```
```json
{
  "answer": "Your two most valuable segments are Injectable VIPs (247 patients, $6,880 avg LTV) and Membership VIPs (57 patients, $6,004 avg LTV)…",
  "sources": [
    { "title": "Patient segment overview", "source": "mart_patient_segments" },
    { "title": "Segment profile — Injectable VIPs", "source": "mart_patient_segments" }
  ]
}
```
- `question` ∈ [1, 1000] chars. `422` if empty/blank or too long.
- `503` if the assistant isn't configured (`ANTHROPIC_API_KEY` unset) or the corpus
  is empty (run `scripts/build_rag_corpus.py`).
- `502` if generation fails upstream.
- The corpus is cached in-process; a rebuild needs an API restart to take effect.
...
| `422` | Invalid query param (bad date, out-of-range `months`/`limit`, bad `min_priority`) or invalid `/chat` question |
| `500` | Warehouse query error — safe message returned, full error logged via structlog |
| `502` | `/chat` generation failed upstream |
| `503` | `/chat` not configured (no `ANTHROPIC_API_KEY`) or corpus empty |
